...
#     sql=("SELECT symbol, date, ret, ret_excess, mktcap, mktcap_lag, "
#         #  "exchange "
#          "FROM prices_monthly"),
#     con=tidy_finance,
#     parse_dates={"date"}
...
def momentum_strategy(data, J, K, n_portfolios=10):
    ...
    # Step 2: Assign to momentum portfolios
    def assign_rank(group):
        try:
            ...
            group["momr"] = pd.cut(
                group["cum_return"].rank(method="first"),
                bins=n_portfolios,
                labels=range(1, n_portfolios + 1)
            ).astype(int)
        return group
    
    df = df.groupby("date", group_keys=False).apply(assign_rank)
    ...
    port_ret = port_ret.query("hdate >= hdate1 and hdate <= hdate2")
    ...
    wide.columns = ["date"] + [f"port{i}" for i in range(1, n_portfolios + 1)]
    wide["winners"] = wide[f"port{n_portfolios}"]
...
def momentum_strategy_vw(data, J, K, n_portfolios=10):
    ...
    port_ret = port_ret.query("mktcap_lag > 0")
...
ew_vs_vw = (ewret_wide[["date", "cumret_long_short"]]
    .rename(columns={"cumret_long_short": "cumret"})
    .assign(scheme="Equally Weighted")
    .append(
        vw_results[["date", "cumret_ls_vw"]]
...
# Keep only daily returns within each holding period
daily_mom_returns = daily_mom_returns.query(
    "ddate >= hdate1 and ddate <= hdate2"
)
