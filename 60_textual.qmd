...
Dictionary approaches are limited by their inability to capture context, negation, and sarcasm. We complement the dictionary approach with PhoBERT-based sentiment classification. We fine-tune ViFinBERT (a financial Vietnamese BERT variant) on a manually labeled dataset of 5,000 Vietnamese financial sentences annotated for sentiment polarity (positive, negative, neutral).
...
| ViFinBERT (fine-tuned) | **0.831** | **0.812** | **0.847** |  **0.824**   |
...
::: callout-note
Standard errors double-clustered by firm and time in parentheses. \*, \*\*, \*\*\* denote significance at 10%, 5%, 1% levels respectively.
:::
...
::: callout-note
\*Requires Vietnamese word segmentation as preprocessing step. VN Support rates how well the method handles Vietnamese text natively.
:::

For researchers beginning textual analysis of Vietnamese firms, we recommend the following workflow:

1.  **Start with TF-IDF cosine similarity** for peer identification---it is fast, interpretable, and provides a strong baseline.
2.  **Use BERTopic with PhoBERT embeddings** for topic discovery---it produces more coherent topics than LDA for Vietnamese text.
