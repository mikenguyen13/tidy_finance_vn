...
# Plotting configuration — academic style (Times New Roman)
plt.rcParams.update({
    'figure.figsize': (10, 6),
    'figure.dpi': 150,
    'font.family': 'serif',
...
The emerging markets literature has documented several stylized facts about institutional ownership that differ from developed market findings. @AggarwalErelFerreira2011 documented that foreign institutional ownership improves corporate governance in emerging markets. For Vietnam specifically, @VuNguyen2021 examined the relationship between ownership structure and firm performance, while @Vo2015 studied the impact of foreign ownership on stock market liquidity.
...
where $R_{j,t}^p$ is the portfolio return. This decomposition, due to @Sirri1998, separates changes in fund assets into investment returns and investor capital allocation decisions. @CovalStafford2007 showed that flow-driven trades create price pressure, with fire sales by funds experiencing redemptions generating significant negative abnormal returns.
...
The output is a quarterly panel of stock-level observations:
...
: Quarter-End Price Panel Variables {#tbl-price-vars}
...
We construct the holdings panel---the Vietnamese equivalent of merging the 13F Type 1 and Type 3 datasets. The key steps are: (i) identify the first available vintage for each shareholder-stock-report date combination; (ii) compute reporting gaps to flag first and last reports; (iii) classify shareholders; (iv) adjust shares for corporate actions.
...
## Application 1: IO Changes and Future Returns {#sec-app-returns}
...
This chapter developed a framework for computing institutional trades, flows, and turnover ratios in the Vietnamese equity marketd. The key contributions include:
...
The pipeline produces several output datasets:
...
: Summary of Pipeline Output Datasets {#tbl-outputs}
